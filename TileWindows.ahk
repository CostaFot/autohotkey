...
GetVisibleWindows() {
    ...
    for hwnd in WinGetList() {
        ...
        if !(style & 0x10000000) ; WS_VISIBLE
            continue
        if (title = "Program Manager" || title = "Windows Input Experience" || title = "NVIDIA GeForce Overlay" || title = "Picture in Picture" || SubStr(title, 1, 6) = "Meet -" || SubStr(title, 1, 3) = "OBS" || title = "PowerDock" || title = "Clock" || title = "KMPClock" || title = "Command Palette")
            continue
        windows.Push(hwnd)
    }
    return windows
